...
Discover first, act second — one broad query, evaluate, stop within 3 commands; accept a
negative result rather than hunting for workarounds. The rule and its false-negative guard
live in CLAUDE.md (§ Discovery-First Rule), which you already have in context. It applies to
reads and queries too, which is why it is no longer duplicated here.

Example of a broad query in this domain: `wxcli device-configurations show --device-id X --key "Lines.*"`
...
`--help` > skills > reference docs > training data. The full rule lives in CLAUDE.md
(§ Source of Truth Precedence), which you already have in context; it applies to every
caller of `wxcli`, not just this agent, so it is no longer duplicated here.
