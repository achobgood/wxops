...
A command-line tool and AI-assisted playbook for provisioning, managing, migrating, and auditing Webex Calling environments. 180 command groups covering the full Webex Calling, admin, device, messaging, meetings, and contact center API surface.
...
- **180 CLI command groups** — provision locations, users, call features, devices, routing, PSTN, messaging, meetings, and contact center resources from the terminal
...
| **CLI** (180 groups) | Tested, self-describing commands generated from 9 OpenAPI specs | **Malformed execution** — the model emits a command string, not hand-rolled HTTP |
...
The natural question for an agent-driven tool: why not expose the API as MCP tools? Because at this surface area — 180 command groups and several hundred individual operations — one tool per endpoint breaks down:
...
This table shows the most commonly used groups. Run `wxcli --help` to see all 180 groups, which also cover admin, device, messaging, meetings, and contact center APIs.
...
│   ├── main.py                   # Entry point — registers 180 command groups
...
The CLI covers 180 command groups across calling, admin, device, messaging, meetings, and contact center APIs. Not all scopes are needed — request only those for the API domains you use.
