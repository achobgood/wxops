...
def generate_appendix(store: MigrationStore) -> str:
    ...
    Returns an HTML fragment (not a full document).  Each section is wrapped
    in ``<details open>`` / ``<summary>`` for collapsibility.  Sections with
...
def _object_inventory(store: MigrationStore) -> str:
    ...
    # Also count CSS and partition (intermediate types)
    ...
    # Per-location breakdown for users and devices
...
def _decision_detail(store: MigrationStore) -> str:
    ...
    for dtype, dlist in sorted(by_type.items()):
        ...
        # Options detail for each decision
        for d in dlist:
            options = d.get("options", [])
            if options:
                lines.append(f'  <p><strong>{_esc(d.get("decision_id", ""))}</strong> options:</p>')
                lines.append('  <ul>')
                for opt in options:
...
def _css_partition_analysis(store: MigrationStore) -> str:
    """Text-based topology: CSS -> partitions via cross-refs."""
    ...
    # Sort partitions within each CSS by ordinal
    ...
    lines = [
        ...
        '  <table>',
        '    <thead><tr><th>CSS</th><th>Partition (ordered)</th></tr></thead>',
        '    <tbody>',
    ]
    ...
    for css_id in sorted(css_map.keys()):
        ...
        lines.append(
            f'      <tr><td>{_esc(css_name)}</td>'
            f'<td>{_esc(", ".join(pt_names))}</td></tr>'
        )
    ...
    lines.append(
        f'  <p>{total_css} CSS(es) referencing {len(all_partitions)} '
...
def _device_inventory(store: MigrationStore) -> str:
    ...
    # Group by tier, then count by model within each tier
    ...
    # Order tiers: native_mpp, convertible, incompatible, then anything else
    ...
    lines = [
        ...
        '  <table>',
        '    <thead><tr>'
        '<th>Compatibility Tier</th><th>Model</th><th>Count</th><th>Action</th>'
        '</tr></thead>',
        '    <tbody>',
    ]
    ...
    lines.append(f'  <p>{total} total device(s).</p>')
...
def _voicemail_analysis(store: MigrationStore) -> str:
    ...
    # Also check for VOICEMAIL_INCOMPATIBLE decisions
    decisions = store.get_all_decisions()
    ...
    if vm_decisions:
        ...
        for d in vm_decisions:
            lines.append(f'    <li>{_esc(d.get("summary", ""))}</li>')
        lines.append('  </ul>')
...
def _data_coverage(store: MigrationStore) -> str:
    ...
    if journal_count == 0:
        lines.append(
            ...
            'No collection errors recorded.</p>'
        )
    else:
        ...
        if error_count > 0:
            ...
            lines.append('  <p>No collection errors recorded.</p>')
