...
def cmd_list(
    ...
    try:
        ...
        raise typer.Exit(1)
    result = result or []
    items = result.get("participants", result if isinstance(result, list) else []) if isinstance(result, dict) else (result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
...
def create_mute(
    call_id: str = typer.Option(None, "--call-id", help="The call identifier of the participant to mute. The conferen"),
    json_body: str = typer.Option(None, "--json-body", help="Full JSON body (overrides other options)"),
    output: str = typer.Option("id", "--output", "-o", help="Output format: id|json"),
    debug: bool = typer.Option(False, "--debug"),
):
...
def create_unmute(
    call_id: str = typer.Option(None, "--call-id", help="The call identifier of the participant to unmute. The confer"),
    json_body: str = typer.Option(None, "--json-body", help="Full JSON body (overrides other options)"),
    output: str = typer.Option("id", "--output", "-o", help="Output format: id|json"),
    debug: bool = typer.Option(False, "--debug"),
):
