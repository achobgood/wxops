...
def create(
    ...
    default_access_code_enabled: bool = typer.Option(None, "--default-access-code-enabled/--no-default-access-code-enabled", help=""),
    default_access_code: str = typer.Option(None, "--default-access-code", help=""),
...
def list_base_stations(
    ...
    try:
        ...
        if "25008" in str(e):
            ...
            typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(1)
    items = result.get("baseStations", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
        print_table(items, columns=[('ID', 'id'), ('Name', 'name'), ('Model', 'model')], limit=limit)
...
def create_handsets(
    location_id: str = typer.Argument(help="locationId"),
    dect_network_id: str = typer.Argument(help="dectNetworkId"),
    line1_member_id: str = typer.Option(None, "--line1-member-id", help=""),
    custom_display_name: str = typer.Option(None, "--custom-display-name", help=""),
    line2_member_id: str = typer.Option(None, "--line2-member-id", help=""),
    json_body: str = typer.Option(None, "--json-body", help="Full JSON body (overrides other options)"),
    debug: bool = typer.Option(False, "--debug"),
):
...
def list_handsets(
    ...
    try:
        ...
        if "25008" in str(e):
            ...
            typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(1)
    items = result.get("handsets", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
        print_table(items, columns=[('ID', 'id'), ('Name', 'name'), ('Model', 'model')], limit=limit)
