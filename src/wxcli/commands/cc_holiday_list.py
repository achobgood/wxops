...
app = typer.Typer(help="Manage Webex Calling holiday-list.")
...
def cmd_list(
    ...
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list"
    params = {}
...
def create(
    ...
    """Create a new Holiday List\n\nExample --json-body:\n  '{"name":"...","version":"...","organizationId":"...","holidaysCount":"...","description":"...","holidays":[{"name":"...","endDate":"...","startDate":"..."}]}'."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list"
    if json_body:
...
def create_bulk(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list/bulk"
    if json_body:
        body = json.loads(json_body)
    else:
        body = {}
...
def list_bulk_export(
    ...
    """Bulk export Holiday List(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list/bulk-export"
    params = {}
    if page is not None:
        params["page"] = page
    if page_size is not None:
...
def show(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list/{id}"
    try:
...
def list_incoming_references(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/holiday-list/{id}/incoming-references"
    params = {}
...
def list_holiday_list(
    ...
    """List Holiday List(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/v2/holiday-list"
    params = {}
    if filter_param is not None:
        params["filter"] = filter_param
    if attributes is not None:
