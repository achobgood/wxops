...
def cmd_list(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/hds/organizations/{organization_id}/database/details"
    params = {}
    if limit > 0:
        params["max"] = limit
    if offset > 0:
...
@app.command("list-multi-tenant")
def list_multi_tenant(
    organization_id: str = typer.Argument(help="organizationId"),
    output: str = typer.Option("table", "--output", "-o", help="Output format: table|json"),
    limit: int = typer.Option(0, "--limit", help="Max results (0=use API default)"),
    offset: int = typer.Option(0, "--offset", help="Start offset"),
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/hds/organizations/{organization_id}/multiTenant"
    params = {}
    if limit > 0:
        params["max"] = limit
    if offset > 0:
