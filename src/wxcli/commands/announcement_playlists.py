...
app = typer.Typer(help="Manage Webex Calling announcement-playlist.")
...
def cmd_list(
    ...
    url = f"https://webexapis.com/v1/telephony/config/announcements/playlists"
    params = {}
    if limit > 0:
        params["max"] = limit
    if offset > 0:
    ...
    try:
        ...
        raise typer.Exit(1)
    result = result or []
    items = result.get("playlists", result if isinstance(result, list) else []) if isinstance(result, dict) else (result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
...
@app.command("create")
def create(
    name: str = typer.Option(..., "--name", help="Unique name for the announcement playlist."),
