...
def update_caller_id(
    ...
    display_detail: str = typer.Option(None, "--display-detail", help="Workspace's caller ID display details. This field has been deprecated. Please use `directLineCallerIdName` and `dialByName` instead."),
...
def list_numbers(
    ...
    emit(items, output=output, fields=fields, columns=[('Extension', 'extension'), ('Primary', 'primary'), ('Action', 'action'), ('Direct Number', 'directNumber'), ('Ring Pattern', 'ringPattern')], limit=limit)
...
def delete_access_codes(
    ...
    if result:
        ...
        emit({"status": "deleted", "id": access_code}, output=output, fields=fields)
...
def update_passcode(
    ...
    if result:
        ...
        emit({"status": "updated", "id": place_id}, output=output, fields=fields)
