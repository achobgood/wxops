...
def create(
    ...
    latitude: str = typer.Option(None, "--latitude", help=""),
    longitude: str = typer.Option(None, "--longitude", help=""),
...
def list_floors(
    ...
    try:
        ...
        if "25008" in str(e):
            ...
            typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(1)
    items = result.get("floors", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
        print_table(items, columns=[("ID", "id"), ("Name", "name")], limit=limit)
...
def create_floors(
    location_id: str = typer.Argument(help="locationId"),
    floor_number: str = typer.Option(None, "--floor-number", help=""),
