...
app = typer.Typer(help="Manage Webex Calling dial-number.")
...
def cmd_list(
    ...
    url = f"https://webexapis.com/v1/organization/{orgid}/v3/dial-number"
    params = {}
...
def list_dial_number_organization(
    ...
    """List Dialed Number Mapping(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number"
    params = {}
    if filter_param is not None:
        params["filter"] = filter_param
    if attributes is not None:
...
def create_bulk(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number/bulk"
    if json_body:
        body = json.loads(json_body)
    else:
        body = {}
...
def list_numbers_only(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number/numbers-only"
    params = {}
    if limit > 0:
        params["max"] = limit
    if offset > 0:
...
def list_bulk_export(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number/bulk-export"
    params = {}
    if page is not None:
        params["page"] = page
    if page_size is not None:
...
def show(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number/{id}"
    try:
...
def list_incoming_references(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/dial-number/{id}/incoming-references"
    params = {}
...
def list_dial_number_v2(
    ...
    """List Dialed Number Mapping(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/v2/dial-number"
    params = {}
    if filter_param is not None:
        params["filter"] = filter_param
    if attributes is not None:
