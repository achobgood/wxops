...
app = typer.Typer(help="Manage Webex Calling team.")
...
def cmd_list(
    ...
    url = f"https://webexapis.com/v1/organization/{orgid}/team"
    params = {}
...
def create(
    ...
    team_type: str = typer.Option(None, "--team-type", help=""),
    system_default: str = typer.Option(None, "--system-default", help=""),
    id_param: str = typer.Option(None, "--id", help=""),
    dialed_number: str = typer.Option(None, "--dialed-number", help=""),
    rank_queues_for_team: str = typer.Option(None, "--rank-queues-for-team", help=""),
    team_status: str = typer.Option(None, "--team-status", help=""),
    name: str = typer.Option(None, "--name", help=""),
    desktop_layout_id: str = typer.Option(None, "--desktop-layout-id", help=""),
    skill_profile_id: str = typer.Option(None, "--skill-profile-id", help=""),
    multi_media_profile_id: str = typer.Option(None, "--multi-media-profile-id", help=""),
    ...
    if json_body:
        ...
        if team_type is not None:
            body["teamType"] = team_type
        if system_default is not None:
            body["systemDefault"] = system_default
        if id_param is not None:
            body["id"] = id_param
        if dialed_number is not None:
            body["dialedNumber"] = dialed_number
        if rank_queues_for_team is not None:
            body["rankQueuesForTeam"] = rank_queues_for_team
        if team_status is not None:
            body["teamStatus"] = team_status
        if name is not None:
            body["name"] = name
        if desktop_layout_id is not None:
            body["desktopLayoutId"] = desktop_layout_id
        if skill_profile_id is not None:
            body["skillProfileId"] = skill_profile_id
        if multi_media_profile_id is not None:
            body["multiMediaProfileId"] = multi_media_profile_id
...
def create_bulk(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/team/bulk"
    if json_body:
        body = json.loads(json_body)
    else:
        body = {}
...
def list_bulk_export(
    ...
    """Bulk export Team(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/team/bulk-export"
    params = {}
    if page is not None:
        params["page"] = page
    if page_size is not None:
...
def create_purge_inactive_entities(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/team/purge-inactive-entities"
    params = {}
...
def show(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/team/{id}"
    try:
...
def list_incoming_references(
    ...
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/team/{id}/incoming-references"
    params = {}
...
def list_team(
    ...
    page: str = typer.Option(None, "--page", help="Defines the number of displayed page. The page number starts"),
    page_size: str = typer.Option(None, "--page-size", help="Defines the number of items to be displayed on a page. If th"),
    supervisor_view: str = typer.Option(None, "--supervisor-view", help="supervisorView flag honours user-profile team access rights"),
    ...
    """List Team(s)."""
    api = get_api(debug=debug)
    url = f"https://webexapis.com/v1/organization/{orgid}/v2/team"
    params = {}
    if filter_param is not None:
        params["filter"] = filter_param
    if attributes is not None:
