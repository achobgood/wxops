...
def cmd_list(
    ...
    try:
        ...
        raise typer.Exit(1)
    items = result.get("accessSummary", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
...
def list_access_detail(
    recording_id: str = typer.Option(None, "--recording-id", help="A unique identifier for the recording."),
    host_email: str = typer.Option(None, "--host-email", help="Email address for the meeting host. This parameter is only u"),
    ...
    try:
        ...
        if "25008" in str(e):
            ...
            typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(1)
    items = result.get("accessDetail", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
        print_table(items, columns=[("ID", "id"), ("Name", "name")], limit=limit)
...
def list_meeting_archive_summaries(
    ...
    try:
        ...
        if "25008" in str(e):
            ...
            typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(1)
    items = result.get("meetingArchiveSummaries", result if isinstance(result, list) else [])
    if output == "json":
        print_json(items)
    else:
        print_table(items, columns=[("ID", "id"), ("Name", "name")], limit=limit)
