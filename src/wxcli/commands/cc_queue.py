...
def create_fetch_by_dynamic_skills_and_skill_profile(
    ...
    """List skill-based Contact Service Queues by dynamic skills and skill profile.\n\n\b\nExample --json-body: '{"skillProfileId":"...","dynamicSkills":[{"skillId":"...","textValue":"...","booleanValue":"...","proficiencyValue":"...","enumSkillValues":"..."}],"userId":"..."}'"""
    if generate_json_body:
...
def create_contact_service_queue(
    ...
    """Create a new Contact Service Queue.\n\n\b\nExample: wxcli cc-queue create-contact-service-queue --name NAME --queue-type INBOUND --check-agent-availability --channel-type TELEPHONY --service-level-threshold SERVICE_LEVEL_THRESHOLD --max-active-contacts MAX_ACTIVE_CONTACTS --max-time-in-queue MAX_TIME_IN_QUEUE --active --routing-type LONGEST_AVAILABLE_AGENT --queue-routing-type TEAM_BASED\n\n\b\nExample --json-body: '{"name":"...","queueType":"INBOUND","checkAgentAvailability":true,"channelType":"TELEPHONY","serviceLevelThreshold":0,"maxActiveContacts":0,"maxTimeInQueue":0,"active":true}'"""
    if generate_json_body:
...
def update_contact_service_queue(
    ...
    """Update specific Contact Service Queue by ID.\n\n\b\nExample: wxcli cc-queue update-contact-service-queue ID --name NAME --queue-type INBOUND --check-agent-availability --channel-type TELEPHONY --service-level-threshold SERVICE_LEVEL_THRESHOLD --max-active-contacts MAX_ACTIVE_CONTACTS --max-time-in-queue MAX_TIME_IN_QUEUE --active --routing-type LONGEST_AVAILABLE_AGENT --queue-routing-type TEAM_BASED\n\n\b\nExample --json-body: '{"name":"...","queueType":"INBOUND","checkAgentAvailability":true,"channelType":"TELEPHONY","serviceLevelThreshold":0,"maxActiveContacts":0,"maxTimeInQueue":0,"active":true}'"""
    if generate_json_body:
