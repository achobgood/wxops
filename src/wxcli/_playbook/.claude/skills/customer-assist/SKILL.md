...
16. **Queue recording schema is VERIFIED (2026-07-14).** `notification`, `repeat`, and `startStopAnnouncement` are real and writable — proven by live PUT-then-GET read-back. A previous audit (2026-07-01) called this schema "fabricated"; that verdict was **wrong**. The audit correctly found these fields are absent from the *call queue* payload, then wrongly concluded the schema was invented. In fact the fields live in the separate `cxEssentials/callRecordings` sub-resource, which the audit never called. The commands (`show-queue-recording`/`update-queue-recording`) were fabricated; the **schema was not**. Full data model: `docs/reference/call-features-additional.md` → "Queue Call Recording — data model".
