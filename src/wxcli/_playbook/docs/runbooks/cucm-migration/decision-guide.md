...
**Triggered by:** `src/wxcli/migration/transform/analyzers/cross_site.py`. Fires when any construct's members, watchers, delegates, destinations, or line appearances resolve to more than one Webex location. Driven by the `CROSS_SITE_RULES` table (18 rows) covering group membership (hunt group, call queue, pickup group, paging group, DECT), relationships (monitoring/BLF, receptionist, executive-assistant, device layout line members, device placement, hot-desk hosts), forwarding/overflow destinations, and shared DN appearances read from `device_has_dn` cross-refs.
**Options:** `migrate_together` (move every listed member in the same wave — the safe default), `proceed_partial` (create now with local members only; a `reconcile_members` op then completes the membership once every member exists — see below), `reassign_home` (build the construct at a different location — see below), or `skip` (do not migrate the construct).
**Answering `reassign_home`:** do **not** type `reassign_home`. Type the **location canonical id** you want the construct built at — e.g. `wxcli cucm decide D0007` answered with `location:branch-office`. List the ids with `wxcli cucm inventory --type location`. The planner then overrides the construct's location field (whatever its name — `location_id` or `location_canonical_id`), persists it back to the store, moves the construct into that location's batch, and logs `Cross-site reassignment: <type> <id> moved <old> → <new>` at INFO. The literal `reassign_home` id is kept in the options list only as documentation of intent — answering with it records the intent and changes nothing. Only the construct's own decision moves the construct; the remote members listed on the same decision are never relocated. Reassignment applies to constructs that have a location of their own (hunt group, call queue, auto attendant, pickup group, paging group, voicemail group, DECT network, device, device profile, receptionist config). It has no effect on the relationship constructs that inherit their owner's location — monitoring/BLF lists, executive-assistant pairings, per-person call forwarding, device layouts — where the only meaningful answers are `migrate_together` or `skip`.
**Recommendation:** none. This type is deliberately excluded from auto-resolution (`_NEVER_AUTO_RESOLVE` in `transform/rules.py`) and from the advisory recommendation layer. A phased migration cannot be made safe by a heuristic — the operator has to say what happens to the people on the other side of the boundary.
**Blocking behaviour:** unique among decision types — an *unresolved* cross-site decision blocks plan expansion of its construct (`reason="cross_site_unreviewed"` in the planner skip report) and trips `wxcli cucm plan --fail-on-unresolved`. Only the construct is gated; its remote members are still provisioned normally.
**Severity:** `HIGH` when 3+ locations are involved, when the relation is `line_appearance` (a shared DN/DID has no cross-system equivalent), or when a membership construct's home location does not hold a strict majority of its members. `MEDIUM` otherwise. Never `LOW`.
**Answering `proceed_partial`:** the group is created in wave 1 with whichever members exist, and a `reconcile_members` operation (tier 9, the last thing that touches a group) rewrites the **full** membership once every member has been provisioned. It is held back by hard `REQUIRES` edges on each member's `user:create`, so across separate wave runs it stays pending and executes on the final one. Supported for hunt groups, call queues, pickup groups, and paging groups. Two operational notes: (1) if any member is still unresolved the op reports `skipped` and writes nothing — these endpoints replace the whole member array, so a partial write would *delete* members, including any an operator added by hand; (2) a user may belong to only one pickup group (error 4471), so a reconcile that would add an already-assigned user fails with that error rather than retrying.
**Cascade impact:** resolving with `proceed_partial` leaves the construct incomplete until the wave that provisions its last remote member — the reconcile op is what closes it. Until then, hunt groups and call queues log a warning per excluded agent at create time, and pickup and paging groups drop them silently.
**See also:** [`shared-line-complex`](#shared-line-complex) (how a shared DN should be modelled — cross-site answers *whether it can be split across waves*), [`location-ambiguous`](#location-ambiguous), Appendix AF of the assessment report (grouped by site pair — the wave-planning input).
...
**Deprecated 2026-04-15 — convertible devices auto-convert, no decision emitted.** If a device's phone model is classified as `CONVERTIBLE` by `DeviceMapper`, the planner unconditionally emits a `create_activation_code` op and the device is re-provisioned on Webex Calling firmware. There is no operator choice, no skip, and no advisory at the decision level — convertibility is a property of the model, not a question for the operator. The `DecisionType.DEVICE_FIRMWARE_CONVERTIBLE` enum value is retained for backward compatibility with project stores created before 2026-04-15; no code path emits it for new runs. SRST-dependency concerns remain captured by the advisor narrative (see [`kb-device-migration.md#dt-dev-002`](../../knowledge-base/migration/kb-device-migration.md#dt-dev-002)) rather than as a per-device decision.
**See also:** [Advisory Patterns §device-bulk-upgrade](#device-bulk-upgrade), [`device-incompatible`](#device-incompatible) (the harder sibling case that still surfaces as a decision), [Tuning Reference §Recipe 5](tuning-reference.md#recipe-5-analog-gateway-heavy-customer).
...
**See also:** [`kb-feature-mapping.md` § Audio Assets Migration](../../knowledge-base/migration/kb-feature-mapping.md), [`docs/reference/location-calling-media.md` § Announcements / MOH](../../reference/location-calling-media.md).
...
| DEVICE_FIRMWARE_CONVERTIBLE | Deprecated 2026-04-15 | n/a — convertible devices auto-convert at plan time (tier=`CONVERTIBLE` → always emit `create_activation_code` op). No decision is emitted for new runs; the enum value is retained only for backward compatibility with pre-2026-04-15 project stores. Operators with heavy SRST dependencies should scan the advisor narrative — there is intentionally no per-decision opt-out. |
