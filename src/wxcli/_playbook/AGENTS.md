...
Detailed context in `.codex/rules/org-health.md` (the file ships; its `paths:` glob targets this repo's own source, so it auto-loads only in a source checkout). Contains: a one-paragraph overview, run instructions, and check category counts (Security Posture, Routing Hygiene, Feature Utilization, Device Health).
...
Detailed context in `.codex/rules/cleanup.md` (the file ships; its `paths:` glob targets `cleanup.py`, so it auto-loads only in a source checkout). Contains: all flags, 13-layer deletion order, and known behaviors (virtual line workaround, location disable propagation, workspace ordering).
