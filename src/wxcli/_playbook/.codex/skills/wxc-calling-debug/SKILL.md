...
This pipes the token into `wxcli configure` and saves it to `~/.wxcli/config.json`. Do NOT use `export WEBEX_ACCESS_TOKEN=...` — environment variables do not persist across Bash tool calls in Codex.
...
If a reference doc was wrong or incomplete, update it per the Reference Doc Sync Protocol in AGENTS.md.
