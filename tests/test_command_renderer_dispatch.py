...
class TestRenderOptionWithEnumChoices:
    ...
    def test_long_enum_shows_help_hint(self):
        many = [f"VAL_{i}" for i in range(13)]  # renderer shows <=12 inline
        field = _make_field(
            name="region",
            python_name="region",
            enum_values=many,
...
class TestRenderOptionWithDescription:
    ...
    def test_description_in_list_query(self):
        # Use a non-pagination name: "max"/"start"/"offset"/"limit" are
        # suppressed in list commands in favor of the generator's unified
        # --limit/--offset flags. Any other query param is rendered with
        # its description intact.
        ep = _make_ep(
            query_params=[
                _make_field(
                    name="displayName",
                    python_name="display-name",
                    field_type="string",
                    description="Filter by display name substring",
                ),
            ],
        )
        code = _render_list_command(ep, {})
        assert "Filter by display name substring" in code
...
class TestRenderRequiredOption:
    def test_required_string_uses_ellipsis(self):
        ...
        code = _render_create_command(ep, {})
        # required body fields render as None + "(required)" help so
        # --json-body can substitute for individual flags
        assert 'typer.Option(None, "--name"' in code
    ...
    def test_required_bool_uses_ellipsis(self):
        ...
        code = _render_create_command(ep, {})
        # same None+(required) contract; bools get the --flag/--no-flag toggle
        assert 'typer.Option(None, "--enabled/--no-enabled"' in code
        assert "(required)" in code
...
class TestRenderSettingsGet:
    def test_settings_get_renders_like_show(self):
        ...
        # Was `print_json(result)` until 2026-07-25, when all six render paths
        # were moved onto the shared `_render_output_options` + `emit()` pair so
        # every command gained --output/--fields (tools/CLAUDE.md, Decision
        # Record). `emit` IS the rendering contract now; print_json survives
        # only inside create's id-extraction branch.
...
class TestRenderCommandFileDispatch:
    def test_all_command_types_dispatch(self):
        ...
        for ep in endpoints:
            # Match the decorator's opening only: the renderer appends
            # `short_help=` (since 751010c) and may append further kwargs. An
            # exact-string assertion here is what rotted this file for 8 days
            # short of a year while CI could not see it.
