...
def _seed_decisions(tmp_migrations_dir, project="test-project"):
    ...
    # (DEVICE_FIRMWARE_CONVERTIBLE removed 2026-04-15 — convertibility is now
    # a model classification, not a decision. No D004 seeded.)
...
class TestApplyAuto:
    def test_resolves_only_auto_decisions(self, tmp_migrations_dir):
        ...
        assert result.exit_code == 0
        # D001 (DEVICE_INCOMPATIBLE) and D002 (MISSING_DATA on incompatible
        # device via enrichment) match default auto-rules.
        assert "Auto-applied 2 decisions" in result.output

        # Verify: D001, D002 resolved; D003 still pending.
