...
def _route_list(name: str, route_groups: list[str] | None = None) -> MigrationObject:
    """Build a normalized route_list object."""
    return MigrationObject(
        canonical_id=f"route_list:{name}",
        provenance=_provenance(source_id=f"uuid-rl-{name}", name=name),
        status=MigrationStatus.NORMALIZED,
        pre_migration_state={
            "route_list_name": name,
            "description": f"Route list {name}",
            "route_groups": route_groups or [],
        },
    )
...
class TestMapRouteLists:
    def test_single_route_group_creates_canonical(self):
        """Route list with 1 RG member -> 1 CanonicalRouteList, no decision."""
        store = _make_store()
        rg = _route_group("US-West-RG", trunks=[{"trunk_name": "SIP-Trunk-1", "priority": 1}])
        store.upsert_object(rg)
        rl = _route_list("US-West-RL", route_groups=["US-West-RG"])
        store.upsert_object(rl)
        store.add_cross_ref("route_group:US-West-RG", "route_list:US-West-RL", "route_group_to_route_list")

        mapper = RoutingMapper()
        result = mapper.map(store)

        route_lists = [
            obj for obj in store.get_objects("route_list")
            if obj.get("status") == "analyzed"
        ]
        assert len(route_lists) == 1
        assert route_lists[0]["name"] == "US-West-RL"
        assert route_lists[0]["route_group_id"] == "route_group:US-West-RG"
        assert route_lists[0]["cucm_route_groups"] == ["US-West-RG"]

    def test_multi_route_group_creates_decision(self):
        """Route list with 3 RG members -> FEATURE_APPROXIMATION decision."""
        store = _make_store()
        for name in ["RG-A", "RG-B", "RG-C"]:
            rg = _route_group(name, trunks=[{"trunk_name": f"Trunk-{name}", "priority": 1}])
            store.upsert_object(rg)
        rl = _route_list("Multi-RL", route_groups=["RG-A", "RG-B", "RG-C"])
        store.upsert_object(rl)
        ...
        fa_decisions = [
            ...
            and d.context.get("reason") == "multi_route_group_route_list"
        ]
        assert len(fa_decisions) == 1
        assert "Multi-RL" in fa_decisions[0].summary
        assert len(fa_decisions[0].options) == 3

    def test_disabled_route_list_skipped(self):
        """Route list with routeListEnabled=False is skipped."""
        store = _make_store()
        rl = MigrationObject(
            canonical_id="route_list:Disabled-RL",
            provenance=_provenance(source_id="uuid-rl-disabled", name="Disabled-RL"),
            status=MigrationStatus.NORMALIZED,
            pre_migration_state={
                "route_list_name": "Disabled-RL",
                "route_groups": ["RG-A"],
                "routeListEnabled": "false",
            },
        )
        store.upsert_object(rl)
        ...
        assert len(analyzed_rls) == 0

    def test_empty_route_groups_creates_missing_data(self):
        """Route list with no route group members -> MISSING_DATA decision."""
        store = _make_store()
        rl = _route_list("Empty-RL", route_groups=[])
        store.upsert_object(rl)
        ...
        md_decisions = [
            d for d in result.decisions
            if d.type.value == "MISSING_DATA"
            and d.context.get("reason") == "route_list_no_members"
        ]
        assert len(md_decisions) == 1

    def test_full_pipeline_with_route_list(self):
        """End-to-end: trunk -> route group -> route list -> dial plan (points to RG)."""
        store = _make_store()
        ...
        # Route list with single route group
        rl = _route_list("RL-Primary", route_groups=["RG-Primary"])
        store.upsert_object(rl)

        # Route pattern targeting the route list
        rp = _route_pattern(
            "9.1[2-9]XXXXXXXXX",
            target_type="route_list",
            target_name="RL-Primary",
        )
        store.upsert_object(rp)
        store.add_cross_ref(rp.canonical_id, "route_list:RL-Primary", "route_pattern_uses_route_list")

        mapper = RoutingMapper()
        result = mapper.map(store)

        # Route list should be created
        analyzed_rls = [
            obj for obj in store.get_objects("route_list")
            if obj.get("status") == "analyzed"
        ]
        assert len(analyzed_rls) == 1
        assert analyzed_rls[0]["route_group_id"] == "route_group:RG-Primary"

        # Dial plan should still point to route_group (not route_list)
        # because Webex dial plans don't support route_list as RouteType
        dps = store.get_objects("dial_plan")
        assert len(dps) == 1
        assert dps[0]["route_type"] == "ROUTE_GROUP"
        assert dps[0]["route_id"] == "route_group:RG-Primary"
...
class TestRouteListModel:
    def test_canonical_route_list_defaults(self):
        """CanonicalRouteList has correct default values."""
        rl = CanonicalRouteList(
            canonical_id="route_list:US-West-RL",
            provenance=_provenance(source_id="uuid-rl-us-west", name="US-West-RL"),
        )
        assert rl.name == ""
        assert rl.location_id is None
        assert rl.route_group_id is None
        assert rl.numbers == []
        assert rl.cucm_route_list_name == ""
        assert rl.cucm_route_groups == []

    def test_canonical_route_list_with_values(self):
        """CanonicalRouteList stores all fields."""
        rl = CanonicalRouteList(
            canonical_id="route_list:US-West-RL",
            provenance=_provenance(source_id="uuid-rl-us-west", name="US-West-RL"),
            name="US-West-RL",
            ...
            cucm_route_list_name="US-West-RL",
            cucm_route_groups=["US-West-RG", "US-East-RG"],
        )
        assert rl.name == "US-West-RL"
        assert rl.route_group_id == "route_group:US-West-RG"
        assert len(rl.numbers) == 2
        assert rl.cucm_route_groups == ["US-West-RG", "US-East-RG"]
