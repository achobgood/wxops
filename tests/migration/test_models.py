...
class TestAllCanonicalTypes:
    ...
    def test_registry_has_21_types(self):
        assert len(CANONICAL_TYPE_REGISTRY) == 38
...
# ---------------------------------------------------------------------------
# DECT Phase 2 — model and decision type tests
# ---------------------------------------------------------------------------

def test_dect_network_in_registry():
    """CanonicalDECTNetwork is registered in CANONICAL_TYPE_REGISTRY as 'dect_network'."""
    assert "dect_network" in CANONICAL_TYPE_REGISTRY
    from wxcli.migration.models import CanonicalDECTNetwork
    assert CANONICAL_TYPE_REGISTRY["dect_network"] is CanonicalDECTNetwork


def test_dect_network_design_decision_type_exists():
    """DecisionType.DECT_NETWORK_DESIGN is defined."""
    from wxcli.migration.models import DecisionType
    assert hasattr(DecisionType, "DECT_NETWORK_DESIGN")
    assert DecisionType.DECT_NETWORK_DESIGN.value == "DECT_NETWORK_DESIGN"


def test_dect_handset_assignment_decision_type_exists():
    """DecisionType.DECT_HANDSET_ASSIGNMENT is defined."""
    from wxcli.migration.models import DecisionType
    assert hasattr(DecisionType, "DECT_HANDSET_ASSIGNMENT")
    assert DecisionType.DECT_HANDSET_ASSIGNMENT.value == "DECT_HANDSET_ASSIGNMENT"


def test_canonical_dect_network_instantiation():
    """CanonicalDECTNetwork instantiates with all required fields and defaults."""
    from wxcli.migration.models import CanonicalDECTNetwork
    net = CanonicalDECTNetwork(
        canonical_id="dect_network:001",
        provenance=Provenance(
            source_system="cucm", source_id="dect-pk-1", source_name="DECT-HQ",
            extracted_at=datetime.now(timezone.utc),
        ),
        location_canonical_id="loc:001",
        network_name="HQ-DECT",
        display_name="HQ DECT Network",
        model="DBS-110",
        access_code="1234",
        base_stations=[{"mac": "AABB112233", "display_name": "BS-1"}],
        handset_assignments=[{"index": 1, "line1_canonical_id": "line:001"}],
    )
    assert net.canonical_id == "dect_network:001"
    assert net.network_name == "HQ-DECT"
    assert net.model == "DBS-110"
    assert net.access_code == "1234"
    assert len(net.base_stations) == 1
    assert len(net.handset_assignments) == 1
    assert net.pre_migration_state is None
