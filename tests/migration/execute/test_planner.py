...
from wxcli.migration.execute.planner import expand_to_operations, _expand_route_list
...
class TestAdditionalDecisionSkips:
    ...
    def test_convertible_emits_activation_code_unconditionally(self, store):
        """Convertible device → single device:create_activation_code op.
        Emitted unconditionally (no decision lookup); convertibility is a
        model classification, not an operator choice."""
        device = _set_analyzed(CanonicalDevice(
        ...
        assert len(device_ops) == 1  # create_activation_code only (phone self-configures)
        assert device_ops[0].op_type == "create_activation_code"
        assert device_ops[0].resource_type == "device"
        assert device_ops[0].tier == 3
        assert device_ops[0].batch == "location:hq"
        assert "user:alice:create" in device_ops[0].depends_on

    def test_convertible_without_decision_still_emits_activation_code(self, store):
        """Convertible device with no DEVICE_FIRMWARE_CONVERTIBLE decision in the
        store still emits a create_activation_code op — the planner does not
        consult any decision for convertibility."""
        device = _set_analyzed(CanonicalDevice(
            canonical_id="device:replace_me",
            provenance=_prov(),
            mac="AABBCCDDEE00",
        ...
        ops = expand_to_operations(store)
        device_ops = [op for op in ops if op.canonical_id == "device:replace_me"]
        assert len(device_ops) == 1
        assert device_ops[0].op_type == "create_activation_code"

    def test_native_mpp_still_uses_mac_based_create(self, store):
        """NATIVE_MPP devices still emit create + configure_settings (unchanged)."""
        device = _set_analyzed(CanonicalDevice(
            canonical_id="device:native_phone",
            provenance=_prov(),
            mac="112233445566",
            model="DMS Cisco 9861",
            compatibility_tier=DeviceCompatibilityTier.NATIVE_MPP,
        ...
        ops = expand_to_operations(store)
        device_ops = sorted(
            [op for op in ops if op.canonical_id == "device:native_phone"],
            key=lambda op: op.op_type,
        )
        assert [op.op_type for op in device_ops] == ["configure_settings", "create"]

    def test_mac_present_emits_device_create(self, store):
        """Device with MAC → emits device:create op (unchanged behavior)."""
        device = _set_analyzed(CanonicalDevice(
            canonical_id="device:with_mac",
            provenance=_prov(),
            mac="AABBCCDDEEFF",
            model="DMS Cisco 9861",
            compatibility_tier=DeviceCompatibilityTier.NATIVE_MPP,
        ...
        ops = expand_to_operations(store)
        device_ops = [op for op in ops if op.canonical_id == "device:with_mac"]
        op_types = {op.op_type for op in device_ops}
        assert "create" in op_types

    def test_no_mac_drops_device_create(self, store):
        """Device without MAC on the create path → returns [] and logs a warning."""
        device = _set_analyzed(CanonicalDevice(
            canonical_id="device:cti_port",
            provenance=_prov(),
            mac=None,
            model="CTI Port",
            compatibility_tier=DeviceCompatibilityTier.NATIVE_MPP,
        ...
        ops = expand_to_operations(store)
        device_ops = [op for op in ops if op.canonical_id == "device:cti_port"]
        assert device_ops == []

    def test_no_mac_activation_code_path_still_emits(self, store):
        """Convertible device without MAC → still emits create_activation_code (no guard)."""
        device = _set_analyzed(CanonicalDevice(
            canonical_id="device:convertible_no_mac",
            provenance=_prov(),
            mac=None,
            model="DMS Cisco 8851",
            compatibility_tier=DeviceCompatibilityTier.CONVERTIBLE,
        ...
        ops = expand_to_operations(store)
        device_ops = [op for op in ops if op.canonical_id == "device:convertible_no_mac"]
        assert len(device_ops) == 1
        assert device_ops[0].op_type == "create_activation_code"
...
class TestExpandRouteList:
    def test_route_list_produces_create_op(self):
        """Route list with no numbers -> 1 op: create."""
        obj = {
        ...
        ops = _expand_route_list(obj)
        assert len(ops) == 1
        assert ops[0].canonical_id == "route_list:US-West-RL"
        assert ops[0].op_type == "create"
        assert ops[0].resource_type == "route_list"
        assert "route_group:US-West-RG:create" in ops[0].depends_on

    def test_route_list_with_numbers_produces_two_ops(self):
        """Route list with numbers -> 2 ops: create + configure_numbers."""
        obj = {
            "canonical_id": "route_list:US-West-RL",
            "name": "US-West-RL",
            "route_group_id": "route_group:US-West-RG",
        ...
        assert len(ops) == 2
        assert ops[0].op_type == "create"
        assert ops[1].op_type == "configure_numbers"
        assert "route_list:US-West-RL:create" in ops[1].depends_on

    def test_route_list_no_route_group(self):
        """Route list with no route_group_id -> create with no RG dep."""
        obj = {
            "canonical_id": "route_list:Orphan-RL",
            "name": "Orphan-RL",
            "numbers": [],
        }
        ops = _expand_route_list(obj)
...
# ---------------------------------------------------------------------------
# Integration test — real analyzer → planner chain
# ---------------------------------------------------------------------------

def test_convertible_device_with_analyzer_produces_activation_code_op(tmp_path):
    """End-to-end: real DeviceCompatibilityAnalyzer → planner → create_activation_code op.

    As of 2026-04-15 the analyzer produces NO decision for convertible
    devices (convertibility is a model classification). The planner emits
    the create_activation_code op purely from the device's compatibility_tier.
    """
    from datetime import datetime, timezone
    from wxcli.migration.transform.analyzers.device_compatibility import (
    ...
    try:
        ...
        device = _set_analyzed(CanonicalDevice(
            ...
            cucm_device_name="SEP001122AABBCC",
        ))
        store.upsert_object(device)

        # Run the REAL analyzer — should produce NO convertible decisions
        analyzer = DeviceCompatibilityAnalyzer()
        decisions = analyzer.analyze(store)
        convertible = [
            d for d in decisions
            if d.type.value == "DEVICE_FIRMWARE_CONVERTIBLE"
        ]
        assert not convertible, (
            "Analyzer should NOT produce DEVICE_FIRMWARE_CONVERTIBLE decisions — "
            "convertibility is a classification, not a choice"
        )

        # Planner should still emit create_activation_code for the convertible device
        ops = expand_to_operations(store)
        create_ac_ops = [
            op for op in ops
            if op.op_type == "create_activation_code"
            and op.canonical_id == "device:SEP001122AABBCC"
        ]
        assert len(create_ac_ops) == 1, (
            f"Expected 1 create_activation_code op, got {len(create_ac_ops)}. "
            f"All ops: {[(op.canonical_id, op.op_type) for op in ops]}"
        )
    finally:
        store.close()
