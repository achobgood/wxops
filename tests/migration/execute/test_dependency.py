...
class TestRouteListDependency:
    def test_route_list_depends_on_route_group(self):
        """route_list:create depends on route_group:create via intra-expander deps."""
        ops = [
            _op("route_group:US-West-RG", "create", "route_group", 1),
            _op("route_list:US-West-RL", "create", "route_list", 1,
                depends_on=["route_group:US-West-RG:create"]),
            _op("route_list:US-West-RL", "configure_numbers", "route_list", 1,
                depends_on=["route_list:US-West-RL:create"]),
        ]
        G = build_dependency_graph(ops)
        assert G.has_edge("route_group:US-West-RG:create", "route_list:US-West-RL:create")
        assert G.has_edge("route_list:US-West-RL:create", "route_list:US-West-RL:configure_numbers")

    def test_route_list_no_deps_standalone(self):
        """route_list:create with no depends_on has no incoming edges."""
        ops = [
            _op("route_list:Orphan-RL", "create", "route_list", 1),
        ]
        G = build_dependency_graph(ops)
        assert G.number_of_nodes() == 1
        assert G.number_of_edges() == 0
