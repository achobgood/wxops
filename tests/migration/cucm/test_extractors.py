...
class TestDeviceExtractor:
    ...
    def test_device_extractor_captures_speed_dials_and_blf(self):
        """Phone with speeddials and busyLampFields — data preserved."""
        mock_conn = MagicMock()

        phone_with_blf = copy.deepcopy(PHONE_FIXTURE)
        phone_with_blf["speeddials"] = {
            "speeddial": [
                {"index": "1", "dirn": "5551234", "label": "Bob"},
            ]
        }
        phone_with_blf["busyLampFields"] = {
            "busyLampField": [
                {"index": "1", "blfDest": "1002", "label": "Jane", "blfDirn": "1002"},
            ]
        }

        mock_conn.paginated_list.return_value = [{"name": "SEP001122334455"}]
        mock_conn.get_detail.return_value = phone_with_blf
        ...
        phones = extractor.results["phones"]
        assert len(phones) == 1
        assert "speeddials" in phones[0]
        assert "busyLampFields" in phones[0]
        assert len(phones[0]["speeddials"]["speeddial"]) == 1
        assert len(phones[0]["busyLampFields"]["busyLampField"]) == 1
