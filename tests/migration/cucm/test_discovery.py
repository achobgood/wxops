...
class TestRunDiscovery:
    ...
    def test_run_discovery_all_extractors(
        ...
        """All 6 extractors run in order, journal entry written."""
        mock_conn = MagicMock()
        ...
        # All 6 extractors should have been called
        for name in EXTRACTOR_ORDER:
            mock_extractors[name].extract.assert_called_once()

        # Result should contain all 6 extractor results
        assert len(result.extractor_results) == 6
        for name in EXTRACTOR_ORDER:
            assert name in result.extractor_results

        # Total objects should be sum of all extractors
        assert result.total_objects == 3 + 10 + 20 + 15 + 8 + 2
    ...
    def test_run_discovery_version_detection(
        ...
        for MockClass in [MockLocation, MockUser, MockDevice, MockRouting, MockFeature, MockVoicemail]:
    ...
    def test_run_discovery_extractor_failure(
        ...
        # All 6 extractors should still have results (locations with error)
        assert len(result.extractor_results) == 6
        ...
        for name in ["users", "devices", "routing", "features", "voicemail"]:
