...
def _add_bulk_plan_op(store, canonical_id="bulk_device_settings:loc-1",
                      resource_type="bulk_device_settings",
                      op_type="submit"):
    """Add a bulk_* plan_operations row. Skips objects FK (bulk ops don't
    correspond to a canonical object — they synthesize in the planner)."""
    # Disable FK enforcement for this row — bulk_* canonical_ids are synthetic
    store.conn.execute("PRAGMA foreign_keys=OFF")
    store.conn.execute(
        "INSERT OR IGNORE INTO plan_operations "
        "(node_id, canonical_id, resource_type, op_type, tier, batch, status) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (f"{canonical_id}:{op_type}", canonical_id, resource_type, op_type, 5, "site-1", "pending"),
    )
    store.conn.commit()
    store.conn.execute("PRAGMA foreign_keys=ON")


class TestCheckBulkDeviceJobSupport:
    def test_skip_no_bulk_ops(self, store):
        # Plan has per-device ops only — no bulk_* resource types
        store.upsert_object(_analyzed(CanonicalUser(
            canonical_id="user:001", provenance=_prov(), emails=["a@test.com"],
        )))
        _add_plan_op(store, "user:001", resource_type="user")
        result = check_bulk_device_job_support(store, probe_fn=lambda: (200, ""))
        assert result.status == CheckStatus.SKIP
        assert "No bulk device job ops" in result.detail

    def test_skip_when_probe_unavailable(self, store):
        _add_bulk_plan_op(store)
        result = check_bulk_device_job_support(store, probe_fn=None)
        assert result.status == CheckStatus.SKIP
        assert "probe not available" in result.detail

    def test_pass_on_200(self, store):
        _add_bulk_plan_op(store)
        result = check_bulk_device_job_support(store, probe_fn=lambda: (200, ""))
        assert result.status == CheckStatus.PASS
        assert result.data["bulk_op_count"] == 1

    def test_fail_on_404_fedramp(self, store):
        _add_bulk_plan_op(store)
        result = check_bulk_device_job_support(
            store, probe_fn=lambda: (404, "Not Found")
        )
        assert result.status == CheckStatus.FAIL
        assert "Bulk device jobs not supported" in result.detail
        assert "bulk_device_threshold=999999" in result.detail
        assert len(result.issues) == 1
        assert result.issues[0].issue_type == "BULK_JOB_UNSUPPORTED"

    def test_fail_on_403_forbidden(self, store):
        _add_bulk_plan_op(store)
        result = check_bulk_device_job_support(
    ...
    def test_warn_on_unexpected_status(self, store):
        _add_bulk_plan_op(store)
        result = check_bulk_device_job_support(
            store, probe_fn=lambda: (500, "Internal error")
        )
        assert result.status == CheckStatus.WARN
    ...
    def test_counts_all_bulk_types(self, store):
        ...
        for cid, rtype in [
            ("bulk_device_settings:loc-1", "bulk_device_settings"),
            ("bulk_line_key_template:tmpl-1:loc-1", "bulk_line_key_template"),
            ("bulk_dynamic_settings:loc-1", "bulk_dynamic_settings"),
            ("bulk_rebuild_phones:loc-1", "bulk_rebuild_phones"),
        ]:
            _add_bulk_plan_op(store, canonical_id=cid, resource_type=rtype)
        result = check_bulk_device_job_support(store, probe_fn=lambda: (200, ""))
        assert result.status == CheckStatus.PASS
        assert result.data["bulk_op_count"] == 4
