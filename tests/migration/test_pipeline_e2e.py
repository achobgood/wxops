...
class TestPlanningPhase:
    ...
    def test_tier2_snr_planned_report_only_skipped(self, store, raw_data):
        """SNR produces plan operations; data-only types do not.

        device_profile was made executable by the hoteling-migration work
        (enable_hoteling_guest/host). music_on_hold and announcement were
        made visible in the plan by the advisory-to-execution bridge work
        as Phase-A no-op placeholders (api_calls=0, handler returns []).
        e911_config remains data-only because ECBN belongs to
        user:configure_settings and RedSky is a separate workstream.
        """
        self._setup(store, raw_data)
        ops = expand_to_operations(store)
        ...
        # Data-only types should NOT have operations
