...
def populated_store(tmp_path):
    ...
    - 45 devices (37 native MPP CP-8845, 3 DECT Cisco 6825, 3 convertible CP-7841, 2 incompatible CP-7962G)
    ...
    # -- Devices (37 native MPP, 3 DECT, 3 convertible, 2 incompatible) --
    device_specs = (
        [("CP-8845", DeviceCompatibilityTier.NATIVE_MPP)] * 37
        + [("Cisco 6825", DeviceCompatibilityTier.DECT)] * 3
    ...
    # -- Per-user data for user-diff tests --

    # Call forwarding for user-001 (has lossy rules)
    store.upsert_object(CanonicalCallForwarding(
        canonical_id="call_forwarding:user-001",
        provenance=_prov("cf-001", "user-001-fwd"),
        status=MigrationStatus.ANALYZED,
        user_canonical_id="user:user-001",
        always_enabled=False,
        busy_enabled=True,
        busy_destination="1099",
        busy_to_voicemail=True,
        no_answer_enabled=True,
        no_answer_destination="1099",
        no_answer_to_voicemail=True,
        no_answer_ring_count=4,
        busy_internal_enabled=True,
        busy_internal_destination="1099",
        no_coverage_enabled=True,
        no_coverage_destination="1099",
    ))

    # Call forwarding for user-002 (simple, all mapped)
    store.upsert_object(CanonicalCallForwarding(
        canonical_id="call_forwarding:user-002",
        provenance=_prov("cf-002", "user-002-fwd"),
        status=MigrationStatus.ANALYZED,
        user_canonical_id="user:user-002",
        always_enabled=False,
        busy_enabled=True,
        busy_destination="1099",
        busy_to_voicemail=True,
    ...
    # Call forwarding for user-003 (forward always on)
    store.upsert_object(CanonicalCallForwarding(
        canonical_id="call_forwarding:user-003",
        provenance=_prov("cf-003", "user-003-fwd"),
        status=MigrationStatus.ANALYZED,
        user_canonical_id="user:user-003",
        always_enabled=True,
        always_destination="+14155559999",
    ))

    # Wire voicemail profile to users 1-2
    store.add_cross_ref("user:user-001", "voicemail_profile:vm-default", "user_has_voicemail")
    store.add_cross_ref("user:user-002", "voicemail_profile:vm-default", "user_has_voicemail")

    # Monitoring list for user-001 (3 BLF entries, 2 mapped)
    store.upsert_object(CanonicalMonitoringList(
        canonical_id="monitoring_list:user-001",
        provenance=_prov("ml-001", "user-001-blf"),
        status=MigrationStatus.ANALYZED,
        user_canonical_id="user:user-001",
        monitored_members=[
            {"member_id": "user:user-002", "display_name": "User 002", "mapped": True},
            {"member_id": "user:user-003", "display_name": "User 003", "mapped": True},
            {"member_id": "user:external-1", "display_name": "External", "mapped": False},
        ],
    ))

    # Device layout for device-001 (owned by user-001)
    store.upsert_object(CanonicalDeviceLayout(
        canonical_id="device_layout:dev-001",
        provenance=_prov("dl-001", "dev-001-layout"),
        status=MigrationStatus.ANALYZED,
        device_canonical_id="device:dev-001",
        owner_canonical_id="user:user-001",
        resolved_line_keys=[
            {"position": 1, "type": "PRIMARY_LINE", "dn": "1001"},
            {"position": 2, "type": "MONITOR", "target": "user:user-002"},
            {"position": 3, "type": "MONITOR", "target": "user:user-003"},
        ],
        speed_dials=[
            {"position": 1, "label": "Reception", "number": "1000"},
            {"position": 2, "label": "IT Help", "number": "1200"},
        ],
        unmapped_buttons=[
            {"position": 5, "type": "PRIVACY", "label": "Privacy button"},
        ],
    ))

    # Shared line on DN 1001 (user-001 and user-002)
    store.upsert_object(CanonicalSharedLine(
        canonical_id="shared_line:dn-1001",
        provenance=_prov("sl-001", "shared-1001"),
        status=MigrationStatus.ANALYZED,
        dn_canonical_id="line:dn-1001",
        owner_canonical_ids=["user:user-001", "user:user-002"],
        device_canonical_ids=["device:dev-001", "device:dev-002"],
    ))

    # Calling permission for user-001 and user-002 (CSS-Internal mapping)
    store.upsert_object(CanonicalCallingPermission(
        canonical_id="calling_permission:css-internal",
        provenance=_prov("cp-001", "CSS-Internal-perms"),
        status=MigrationStatus.ANALYZED,
        calling_permissions=[
            CallingPermissionEntry(call_type="INTERNAL_CALL", action="ALLOW"),
            CallingPermissionEntry(call_type="NATIONAL", action="ALLOW"),
            CallingPermissionEntry(call_type="INTERNATIONAL", action="BLOCK"),
        ],
        assigned_users=["user:user-001", "user:user-002"],
    ))

    # Cross-ref: user has CSS
    store.add_cross_ref("user:user-001", "css:CSS-Internal", "user_has_css")
    store.add_cross_ref("user:user-002", "css:CSS-National", "user_has_css")

    # Decision referencing user-001 via context (for decision-to-user join test)
    store.save_decision({
        "decision_id": store.next_decision_id(),
        "type": DecisionType.FORWARDING_LOSSY.value,
        "severity": "MEDIUM",
        "summary": "User user-001 has busy_internal forwarding not supported in Webex",
        "context": {"user_canonical_id": "user:user-001", "rule_type": "busy_internal"},
        "options": [
            {"id": "accept_lossy", "label": "Accept loss", "impact": "Rule dropped"},
        ],
        "fingerprint": "fwd-lossy-user001",
        "run_id": "20260324T120000-fixture",
    })
