...
class TestParseParameters:
    def test_path_and_query_params(self):
        ...
        path_vars, query_params, _, _, _ = parse_parameters(params, MINI_SPEC)
        assert path_vars == []
    ...
    def test_omit_query_params(self):
        params = MINI_SPEC["paths"]["/widgets"]["get"]["parameters"]
        path_vars, query_params, _, _, _ = parse_parameters(
...
class TestParseTag:
    ...
    def test_skip_seen_operation_ids(self):
        # dedup keys on (operationId, path) — the CC spec reuses operationIds
        # across different resource paths
        seen = {("listWidgets", "/widgets")}
