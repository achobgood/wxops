...
TERM=dumb COLUMNS=400 python3 -m pytest tests/ -m "not live" -q   # 0 — never skip, mark or delete a failing test; never bare `pytest` (in the sandbox it is a uv tool install without typer, measured 2026-09-14)
...
3. Verdict `patch`/`minor` only, first: `python3 -m tools.sync_classify --base origin/main --notes .spec-sync --version vX.Y.Z > docs/spec-sync/release-notes.md && git add docs/spec-sync/release-notes.md`. The notes ride in the sync commit because `release.yml` reads them from the released commit (their first line must be `# vX.Y.Z `, or it refuses). On a `noop`/`specs-only` week leave the file alone. Then one commit: `chore(specs): sync + regen (<date>) — <verdict>, <N> groups / <M> commands`. Stage by path: `git add -u specs src/wxcli/commands src/wxcli/_playbook tools/spec_semantics.json tools/command_name_lock.json tools/field_overrides.yaml docs/arch/deliberate-gaps.md docs/spec-sync/last-run.json CLAUDE.md README.md docs/reference .claude/skills`. Never `git add -A`.
...
6. Verdict `patch`/`minor` only: `git push origin HEAD:refs/heads/release/vX.Y.Z`. **Never push a tag** — this sandbox's git proxy refuses `refs/tags` with HTTP 403 while accepting branches (measured 2026-09-14; the repo has no tag rules, so there is nothing to grant). `release.yml` runs on that branch push, re-checks CI on the SHA, checks the notes, creates and pushes the annotated tag, publishes to PyPI, creates the GitHub Release from the tag message, and deletes the branch. Confirm with the same poll as step 5 against `workflows/release.yml/runs?head_sha=…` (a run appears within a minute; wait for `completed`, require `success`), then `git ls-remote --tags origin refs/tags/vX.Y.Z` must print a line. A failed run = §4; if the tag reached origin it is burned and next week's `sync_version` skips it, and if it did not, the version is reusable.
...
| `git push` to `origin` — branches | yes | `git push --dry-run origin HEAD:refs/heads/spec-sync-probe-dry-run` → `* [new branch]`; no branch created. Real pushes to `main` and `spec-sync/failed-2026-09-14` succeeded in run `cse_01KieicA1ks1xKjjvDoXwjkZ` |
| `git push` to `origin` — tags | **no — HTTP 403** | same run, 2026-09-14: `git push origin v1.7.6` → `RPC failed; HTTP 403` three times, minutes after the `main` push worked. The repo has no rulesets and no tag or branch protection (`gh api …/rulesets` → `[]`), so the refusal is the sandbox's scoped git credential, not GitHub repo policy; no documented setting changes it |
...
Release gate, negative proofs (both refused with `No CI run found for <sha>`, nothing built or published, probe tags deleted): 2026-09-09 via `gh release create` on `v0.0.0-gate-probe` (run 34307687281, the `release: published` trigger); 2026-09-09 via annotated tag push `v0.0.0-gate-probe-2` (run 34309266293, the `push: tags` trigger that replaced it). Positive proof of the release-branch path, 2026-09-14: v1.7.6, pushed as `release/v1.7.6` at 96576f4 from a workstation (run 34885119359, every step success) → tag `v1.7.6` on origin, wheel + sdist on PyPI, GitHub Release created, branch deleted, and exactly one Release run (the tag the workflow pushed with `github.token` started none). Not yet proven: that the cloud sandbox can push a `release/*` branch — it pushed `main` and `spec-sync/failed-2026-09-14` in the same run that was refused the tag.

Consequences applied above: every command is `python3`; the interpreter check does not import `wxcli`; §4 signals failure by pushing a `spec-sync/failed-<date>` branch instead of `gh issue create`; §5.5 waits for CI with `curl`+`jq` instead of `gh run watch`; §5.6 releases by pushing a `release/vX.Y.Z` branch instead of `gh release create` or a tag (`release.yml` triggers on `push: branches: release/v*` since 2026-09-14, creates the tag and the GitHub Release itself; humans keep `push: tags`).
