...
Release gate, negative proofs (both refused with `No CI run found for <sha>`, nothing built or published, probe tags deleted): 2026-09-09 via `gh release create` on `v0.0.0-gate-probe` (run 34307687281, the `release: published` trigger); 2026-09-09 via annotated tag push `v0.0.0-gate-probe-2` (run 34309266293, the `push: tags` trigger that replaced it). Positive proof of the release-branch path is v1.7.6 — expect exactly one Release run per release (the tag the workflow pushes with `github.token` must not start a second).
