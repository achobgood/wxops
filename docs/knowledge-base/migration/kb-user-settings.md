...
- **"selective forward"**: No direct CUCM equivalent — detected via CSS/partition heuristics in `SelectiveCallHandlingAnalyzer`. Maps to Webex Selective Forward (`/telephony/config/people/{personId}/selectiveForward`). Admin-configurable.
<!-- Source: person-call-settings-handling.md; advisory_patterns.py detect_selective_call_handling_opportunities() -->

- **"selective accept"**: No direct CUCM equivalent — detected via CSS/partition heuristics. Maps to Webex Selective Accept (`/telephony/config/people/{personId}/selectiveAccept`). Admin-configurable.
<!-- Source: person-call-settings-handling.md -->

- **"selective reject"**: No direct CUCM equivalent — detected via CSS/partition heuristics. Maps to Webex Selective Reject (`/telephony/config/people/{personId}/selectiveReject`). Admin-configurable.
<!-- Source: person-call-settings-handling.md -->

- **"priority alert"**: No direct CUCM equivalent — detected via CSS/partition heuristics. Maps to Webex Priority Alert (`/people/me/settings/priorityAlert`). User-only OAuth required — cannot be set by admin token.
<!-- Source: self-service-call-settings.md; CLAUDE.md known issue #4 -->
...
**Current production behavior (2026-04-11):** `_infer_license_tier()` reads `call_recording_enabled`, `voicemail_enabled`/`custom_greetings`, and `outgoing_call_permissions` from the phone state, but none of those fields are populated by the current normalizers — so **every inferred workspace defaults to Workspace (basic) tier** under default inference. The license gate then strips everything except `doNotDisturb`. Voicemail, callForwarding, and privacy settings only reach the Webex API when an operator overrides the `WORKSPACE_LICENSE_TIER` decision to "Professional Workspace" during decision review (`{"type": "WORKSPACE_LICENSE_TIER", "choice": "professional"}` in auto-rules or manual resolution). Field mappings to real CUCM AXL fields have been verified against live CUCM 14.0: `dndStatus`/`dndOption` on phone root, `callInfoPrivacyStatus` on phone root, `voiceMailProfileName` per-line (as a zeep reference dict), and `callForward{All,Busy,NoAnswer}` per-line.
