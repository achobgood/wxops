...
**Current production behavior (2026-04-11):** `_infer_license_tier()` reads `call_recording_enabled`, `voicemail_enabled`/`custom_greetings`, and `outgoing_call_permissions` from the phone state, but none of those fields are populated by the current normalizers — so **every inferred workspace defaults to Workspace (basic) tier**. The license gate then strips everything except `doNotDisturb`. Voicemail, callForwarding, and privacy settings only reach the Webex API if an operator manually overrides the `WORKSPACE_LICENSE_TIER` decision to "Professional Workspace" during decision review. Additionally, `privacy` extraction reads a phone-root field that is not currently returned by `getPhone` (tracking alongside `CallSettingsMapper`'s `enablePrivacy` gap) — privacy migration is a no-op until `PHONE_GET_RETURNED_TAGS` is expanded with live-verified AXL field names.
