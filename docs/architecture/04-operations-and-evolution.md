...
wxcli is published to **public PyPI** as `wxcli` (`pipx install wxcli` / `pip install wxcli`). Releases are built and shipped by `.github/workflows/release.yml`, which triggers on a `push` of a `v*` tag (humans) or a `release/v*` branch (the weekly spec-sync routine, whose sandbox cannot push tags — the workflow creates the tag): it gates on green CI for the exact SHA, checks out full history (`fetch-depth: 0`, required for setuptools-scm version stamping), builds a wheel + sdist, runs a **version guard** (built version must equal the tag with `v` stripped, no `.dev`/`+local` segment), publishes via **Trusted Publishing** (OIDC — no stored token), and attaches the artifacts to the GitHub Release. Release notes are authored in the GitHub web UI *before* publishing, so one action ties notes + artifact + PyPI together.
...
**Async for bulk operations.** The migration engine established the pattern: sync `httpx` for single-command CLI work, async `aiohttp` for batch operations. **Do not build on `src/wxcli/migration/rate_limiter.py`** — an earlier version of this sentence recommended it, and it is dead code: nothing under `src/` imports it, and none of its five declared parameters matches what actually runs. The live policy is inline in `migration/execute/engine.py` — concurrency comes from `wxcli cucm execute -c` (default **20**, not `RateLimitConfig`'s 5), and only HTTP 429 is retried, with `Retry-After` slept verbatim (uncapped, unjittered, unguarded `int()`). That policy disagrees with the shared runtime's in six measured ways; see `docs/audit/06-machinery.md` §3. Until that is reconciled, a new bulk module should reuse `wxcli/auth.py`'s retry constants (`RETRY_STATUSES`, `MAX_RETRY_AFTER_SECONDS`, `_backoff_delay`) rather than either existing implementation.
