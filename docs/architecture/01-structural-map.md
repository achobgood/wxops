...
| `src/wxcli/commands/` | 172 generated command modules + 6 hand-written — each exports a Typer sub-app | Generated modules are mounted via the generator-emitted `_registry.py` manifest loop in `main.py`; hand seams, aliases, and the dev-only `fs_*` block stay explicit (ADR-9) |
...
| `specs/` | 9 tracked OpenAPI 3.0 JSON specs (+1 dev-only untracked) — the source of truth for API surface | Synced + regenerated atomically via `tools/spec_sync.py`; read by `tools/openapi_parser.py` and `tools/generate_commands.py` |
...
1. **Generated vs hand-coded commands.** 171 modules are generated from OpenAPI specs and registered via the generator-emitted `_registry.py` manifest (ADR-9); 6 files are hand-coded: `update.py` (self-update via git pull), `configure.py` (auth setup), `cucm.py` (migration CLI surface), `cleanup.py` (1,424-line batch deletion with 13-layer dependency ordering), plus `converged_recordings_export.py` (mounts `download`/`export` onto the generated `converged-recordings` group via the manifest loop's app dict). `cucm_config.py` is a helper mounted by `cucm.py`. The legacy trio is fully retired: `locations.py`/`numbers.py` turned out to be generator-owned on disk (2026-07-01), and `licenses.py` was consolidated into the generated `licenses` group on 2026-07-02 (S3.1, live-verified parity; `licenses-api` alias kept one release). The seam is `main.py`: manifest loop for generated groups, explicit `app.add_typer()` for hand seams/aliases/dev-only `fs_*`.
