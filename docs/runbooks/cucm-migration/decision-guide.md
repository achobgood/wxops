...
|---|---|---|
| AUDIO_ASSET_MANUAL | Conditional | Customer-facing usage (`usage_type` in `AA_GREETING`, `QUEUE_COMFORT`, `QUEUE_WHISPER`) where the recommender returns `accept` — those assets need real audio files staged for manual re-upload, which is a per-asset operator task. Bulk-accept is only safe for MOH tracks confined to 1-2 locations where `use_default` is appropriate. |
...
| DN_AMBIGUOUS | No | n/a — Webex requires a single explicit owner per DN. Every ambiguous DN has to be resolved to one of `{assign to user, treat as extension-only, treat as DID, skip}` with direct knowledge of how that number is actually used. |
...
| FEATURE_APPROXIMATION | Conditional | Hunt pilots with `agent_count` between 5 and 8 AND no queue-feature indicators (`has_queue_features=False`) — the recommender returns `None` for this ambiguous band because HG vs CQ depends on customer intent, not measurable signal. Also disqualified: CTI Route Points where `complex_script=True`, and any hunt pilot exceeding `policy_agent_limit` with unclear routing semantics. |
...
| LOCATION_AMBIGUOUS | No | n/a — location is the highest-fan-out resolution in the pipeline. Every customer has a different site-to-device-pool mapping, timezone/region boundaries, and E911-address constraints. Bulk-accepting would mis-bind devices, users, schedules, and features across locations. The `no address + dependent devices` case specifically requires human-supplied data. |
...
| SHARED_LINE_COMPLEX | Conditional | `appearance_count > 10` OR `unique_owners >= 2` with mixed-usage secondary labels (labels do NOT all match `BLF`/`Monitor`/`Busy Lamp`/`Speed`/`DSS` keywords). In both disqualifying cases the `recommend_shared_line_complex` rule returns `None` because the line is carrying a mix of monitoring and actual shared-appearance usage and the right answer depends on operator intent. Pure-monitoring groups (all secondary labels match monitoring keywords) are safe to bulk-accept as `virtual_extension`. |
...
| VOICEMAIL_INCOMPATIBLE | Conditional | Unity Connection profile carries features with no Webex equivalent (`unity_features` list is non-empty with items like extensive call-handling rules, transfer-to-number routing, or notification workflows). The `recommend_voicemail_incompatible` rule always returns `webex_voicemail`, but bulk-accepting is only safe when the lost Unity features are trivial — when the list contains call-handling rules or transfer logic, every user needs an individual review of whether Webex voicemail is actually adequate. |
