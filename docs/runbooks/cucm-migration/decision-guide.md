...
**Audit trail.** Every dissent the agent emits cites a specific KB section via the `KB: <doc> section <section name>` tag. Once Wave 4 lands the anchor backfill into the KB files, those section names will normalize to short-form lowercase IDs in the form `dt-<domain>-NNN` — for example `dt-routing-001`, `dt-dev-003`, `dt-user-005`, `dt-trunk-002`, `dt-id-001`. The [§Decision Types A–Z](#decision-types-az) and [§Advisory Patterns](#advisory-patterns) sections already cite those IDs in their **See also** footers (e.g. `kb-css-routing.md#dt-routing-002`), so operators can trace the evidence base the advisor is reasoning from.

**Today (pre-Wave-4):** the `dt-<domain>-NNN` IDs exist as link targets in [§Decision Types A–Z](#decision-types-az) but the corresponding KB anchors haven't been backfilled yet — `grep` against the KB files will return zero matches. After Wave 4 Phase E lands, you'll be able to run:

```bash
grep -n 'dt-routing-002' docs/knowledge-base/migration/kb-css-routing.md
```

and see the dissent-trigger evidence directly in the KB file. Until then, read the [§Decision Types A–Z](#decision-types-az) entry that cites the ID — its **Dissent triggers** and **See also** lines paraphrase the same KB content the anchor backfill will eventually expose.
