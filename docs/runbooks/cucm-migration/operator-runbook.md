...
### Bulk execution (1000+ devices)

The migration engine automatically switches to Webex bulk job APIs when
the plan contains at least `bulk_device_threshold` device creations
(default: 100). Instead of issuing a PUT per device for settings and
layouts, the engine submits a single bulk job per location and polls
the job until completion.

**What you'll see in the progress log:**

```
Batch: location:sf-hq / tier 5 (1 ops)
  bulk_device_settings:location:sf-hq:submit: in_progress
Job callDeviceSettings JOB_...: 20% complete
Job callDeviceSettings JOB_...: 60% complete
Job callDeviceSettings JOB_... exit=COMPLETED updated=483
...
(rebuildPhones is not supported) or when you want per-device error
granularity.

**Time savings:** 1000 phones ≈ 80 min → 19 min with bulk enabled (spec
§5d). Savings scale with device count.
